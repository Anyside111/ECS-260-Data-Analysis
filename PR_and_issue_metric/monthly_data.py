...
def calculate_monthly_issue_stats(issues_by_month):
    """
    Calculate monthly stats for issues.
    """
    monthly_stats = {}
...
def process_and_save_data(excel_path):
    ...
    # 初始化或加载已有的数据
    try:
        with open('Irene_proj_metrics.json', 'r') as file:
    ...
    for index, row in df.iterrows():
        ...
        if repo not in all_data:
            ...
            # 立即将更新后的数据保存为JSON文件
            with open('Irene_proj_metrics.json', 'w') as file:
                json.dump(all_data, file, indent=2)
            print(f"Data for {repo} has been saved to all_repos_data.json")
            repo_count += 1

# 示例调用
excel_path = 'D:/ECS 260/git_mining/ASFI List_reduced.xlsx'
